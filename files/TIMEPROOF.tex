...
# Constants
<locktime> OP_CHECKLOCKTIMEVERIFY OP_DROP  # Minimum verification time (1 year)
<min_bounty> OP_8 OP_VERIFY                # Minimum bounty (1 BTC in satoshis)

# Proof Structure
<proofHash> OP_HASH160                      # proofHash
<escrow_address> OP_HASH160                 # escrow address
OP_0                                        # verified flag
<bitcoinTxHash> OP_SHA256                   # bitcoinTxHash
...
OP_IF
    ...
    # Verify signature
    <signature> <pubkey> OP_CHECKSIGVERIFY

    # Ensure minimum bounty
    <min_bounty> OP_LESSTHANOREQUAL OP_VERIFY
OP_ELSE
    OP_DROP
OP_ENDIF
...
\begin{itemize}
    \item \textbf{Proof Structure}: Uses OP\_HASH160 and OP\_SHA256 for storing proof details
    \item \textbf{Create Proof}: Validates the proof creation with OP\_CHECKSIGVERIFY
